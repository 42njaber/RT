...
void		set_default_scene(t_ptr *p, char *def)
{
	...
	if (def[1] == 1)
	{
		...
		p->objs[0] = (t_obj){0, vec3(20, 0, 50), vec3(0, 0, 0),
			vec3(10, 10, 10), 0x00FFAA00, 0, {0}, {0}, {0}};
		p->objs[1] = (t_obj){1, vec3(0, 0, 70), vec3(0, 0, 0),
			vec3(1, 1, 1), 0x00FF22FF, 0, {0}, {0}, {0}};
		p->objs[2] = (t_obj){1, vec3(0, 20, 0), vec3(90, 0, 0),
			vec3(1, 1, 1), 0x0000AA00, 0, {0}, {0}, {0}};
	}
	if (def[2] == 1)
	{
		p->nspots = 1;
		if ((p->spots = ft_memalloc(sizeof(t_spot) * p->nspots)) == NULL)
			ft_error("[Erreur] Failed to allocate memory\n");
		p->spots[0] = (t_spot){vec3(0, -20, -10), 2000};
	}
}
